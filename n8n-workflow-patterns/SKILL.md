...
- **Headers showing data row values**: If JSON keys are data values instead of column titles, the `dataLocationOnSheet` range is starting AFTER the header row (e.g., `A2:ZZ200` instead of `A1:ZZ200`). n8n treats the first returned row as headers. **Fix**: Change range to include row 1 (`A1:...`) or switch `rangeDefinition` to `detectAutomatically`.
- **sheetName with full URL**: `sheetName.value` should be the sheet gid (e.g., `"1600176412"`) with `mode: "id"`, not the full spreadsheet URL. `documentId` handles the spreadsheet URL.
...
## Debugging n8n Workflows (Fast Path)

When a workflow has an issue, follow this priority order **before** reading source code or guessing:

### 1. Inspect the LIVE workflow via n8n Public API
`workflow_code.js` or local files may be stale. The live n8n instance is the source of truth.

```bash
# Get API key from n8n SQLite DB (if not known)
# Then query the actual workflow:
curl -s http://localhost:5678/api/v1/workflows/{workflowId} \
  -H "X-N8N-API-KEY: $API_KEY" | jq '.nodes[].parameters'
```

**Why this matters**: The workflow in n8n's database often differs from `workflow_code.js` (especially when MCP updates fail or manual UI edits were made). Always verify the live config first.

### 2. Update workflows via Public API (when MCP fails)
If MCP `update_workflow` is unreliable, use the public API directly:

```bash
# GET current workflow, modify nodes, PUT back
curl -s http://localhost:5678/api/v1/workflows/{id} -H "X-N8N-API-KEY: $KEY" | \
  jq '.nodes |= map(if .name == "NodeName" then .parameters.field = "value" else . end)' > /tmp/wf.json

# Strip read-only fields before PUT
cat /tmp/wf.json | jq '{name, nodes, connections, settings, staticData}' | \
  curl -s -X PUT http://localhost:5678/api/v1/workflows/{id} \
    -H "X-N8N-API-KEY: $KEY" -H "Content-Type: application/json" -d @-
```

**Gotcha**: The public API rejects `tags`, `pinData`, `meta`, `versionId`, `activeVersion`, etc. Strip to only: `name`, `nodes`, `connections`, `settings`, `staticData`.
...
### 4. Only read n8n source code as last resort
If the live config looks correct but behavior is still wrong, then (and only then) look at the node implementation in `/usr/local/lib/node_modules/n8n/node_modules/n8n-nodes-base/dist/nodes/...`
...
|---------|------------------|----------|
| "Could not get parameter" on execution | Missing required node parameter (e.g., `operation`, `resource`, or a required field) | Inspect live workflow via API; compare with node docs for required fields |
| JSON keys are data values, not headers (Google Sheets) | `dataLocationOnSheet.range` starts after header row (e.g., `A2:...`) | Change range to `A1:...` or use `detectAutomatically` |
| "Sheet with ID NaN not found" (Google Sheets) | `sheetName.mode: 'url'` with full URL; n8n can't parse sheet ID | Use `mode: 'id'` with just the gid, or `mode: 'list'` with sheet name |
| Filter node passes everything or nothing | Empty `leftValue`/`rightValue` in condition | Configure both sides of the comparison |
| MCP `update_workflow` returns success but changes don't stick | Known n8n MCP limitation; use public API PUT instead | Use `curl -X PUT /api/v1/workflows/{id}` with stripped JSON |
| `validate_workflow` passes but execution fails | `validate_workflow` checks syntax, not parameter completeness | Manually verify all required parameters exist in live workflow |
