...
by calling the CLI tool (`claude -p`) as a subprocess (same auth pattern as run_eval.py —
uses the session's terminal environment auth, no separate API key needed).
"""
...
def _call_llm(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    # Remove CLAUDECODE env var to allow nesting CLI calls inside a
    # terminal session. The guard is for interactive terminal conflicts;
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; the CLI tool is one-shot, so we
